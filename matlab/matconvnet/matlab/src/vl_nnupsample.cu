                 ...
                 int nin, mxArray const *in[])
{
  ...
  while ((opt = vlmxNextOption (in, nin, options, &next, &optarg)) >= 0) {
    switch (opt) {
      ...
      case opt_method:
        ...
        if (vlmxIsEqualToStringI(optarg, "sparse")) {
          sparse = 1;
        } else if (vlmxIsEqualToStringI(optarg, "dense")) {
          sparse = 0;
        } else {
  ...
  if (verbosity > 0) {
    mexPrintf("vl_nnupsample: %s; %s", backMode?"backward":"forward", (data.getMemoryType()==vl::GPU)?"GPU":"CPU") ;
    ...
    mexPrintf("vl_nnupsample: stride: [%d %d]\n",
              strideY, strideX) ;
    mexPrintf("vl_nnupsample: method: %s\n", (sparse == 1)? "sparse" : "dense") ;

    vl::print("vl_nnupsample: data: ", data);
    
    if (backMode) {
      vl::print("vl_nnupsample: derOutput: ", derOutput);
      vl::print("vl_nnupsample: derData: ", derData);
    } else {
      vl::print("vl_nnupsample: output: ", output);
    }
  }
  ...
  vl::Error error;
  if (!backMode) {
    error = vl::nnupsampling_forward(context,
                                    output, data,
                                    sparse,
                                    strideY, strideX);
  } else {
    error = vl::nnupsampling_backward(context,
                                     derData, derOutput,
                                     sparse,
                                     strideY, strideX);
  }
